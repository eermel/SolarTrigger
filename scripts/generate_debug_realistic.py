...
eclipse_title    = "DEBUG Realistic — 2026 Spain eclipse proportions"
eclipse_type     = "Totale"
magnitude        = 1.02618
...
# ── Structure JSON ─────────────────────────────────────────────────────────────
data = {
    "_comment":        "Generated by generate_debug_realistic.py — real 2026 eclipse proportions",
    ...
    "_circumstances_location": {
        ...
        "comment": "DEBUG: shifted circumstances, no real validity position."
    },
...
print(f"  C1      : {fmt(C1)} UTC  (+{dur(C1-TSTART)} — partial phase start)")
print(f"  C2      : {fmt(C2)} UTC  (+{dur(C2-TSTART)} — totality start)")
print(f"  TMAX    : {fmt(TMAX)} UTC  (maximum)")
print(f"  C3      : {fmt(C3)} UTC  (totality end — {int(totality_sec)}s of totality)")
print(f"  C4      : {fmt(C4)} UTC  (partial phase end)")
print(f"  TEND    : {fmt(TEND)} UTC  (+{dur(TEND-TSTART)} total)")
print(f"")
print(f"  Total sequence duration: {dur(total_dur)}")
print(f"  Partial-phase 1a photos  : ~{int((C2-timedelta(seconds=duree_diamond_ring)-C1).total_seconds()//interval_partial)} photos (every {interval_partial}s)")
print(f"  Photos diamond ring  : ~{int((duree_diamond_ring*2)//interval_diamond_ring)} photos (every {interval_diamond_ring}s)")
print(f"  Totality photos      : 15 shutter speeds × 2 passes")
print(f"  Partial-phase 3b photos  : ~{int((C4-C3-timedelta(seconds=duree_diamond_ring)).total_seconds()//interval_partial)} photos (every {interval_partial}s)")
