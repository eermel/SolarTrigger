...
class SonyPlugin(CameraPlugin):
    ...
    def prepare_capture(self, intent):
        ...
        if intent.exposure_plan is not None:
            ...
            shutters = [speed for speed, _iso in plan]
            iso_values = {iso for _speed, iso in plan}

            # Keep the native Sony bracket optimization only when the
            # physical views generated by the Sony planner are EXACTLY
            # the final materialized shutter plan and every view uses
            # the same ISO.
            if len(iso_values) == 1:
                ...
                if regular:
                    ...
                    if self._sequence_shutters(candidate) == shutters:
                        ...
                        return PreparedCapture(
                            ...
                            exposures_s=self._sequence_exposures(candidate),
                            planned_count=self._planned_count(candidate),
            ...
            # Variable ISO, duplicate capped shutters, irregular plan, or a
            # Sony bracket that would add/alter a physical view: exact singles.
            singles = tuple(
