...
class SonyPlugin(CameraPlugin):
        ...
        if intent.exposure_plan is not None:
            ...
            for segment in segments:
                ...
                if regular:
                    ...
                    # A native Sony bracket is accepted only when its actual
                    # physical shutters exactly match this ISO segment.
                    if self._sequence_shutters(candidate) == shutters:
                        sequence = candidate
                        description = (
                            f"materialized exact bracket "
                            f"{fastest}->{slowest} @ {step} IL"
                        )
                ...
                executable_segments.append(
                    (sequence, iso, description)
                )

            # Preserve the existing simple constant-ISO path when the entire
            # exposure plan is one segment.
            if len(executable_segments) == 1:
                sequence, iso, description = executable_segments[0]

                if any(
                    isinstance(item, planner.Bracket)
                    for item in sequence
                ):
                    return PreparedCapture(
                        token=(
                            "sony_exposure_sequence",
                            sequence,
                            intent.deadline,
                            description,
                            iso,
                        ...
                        estimated_total_s=sum(
                            planner.estimate_duration(item)
                            for item in sequence
                        ),
                        exposures_s=[
            ...
            return PreparedCapture(
                token=(
                    "sony_exposure_mixed",
                    tuple(executable_segments),
                    intent.deadline,
                    "materialized hybrid exposure plan",
                ),
                estimated_total_s=sum(
                    planner.estimate_duration(item)
                    for sequence, _iso, _description in executable_segments
                    for item in sequence
                ),
                exposures_s=[
                    planner.parse_speed(speed)
    ...
    def trigger_prepared(self, prepared, deadline=None):
        ...
        if mode == "sony_sequence":
            seq, prepared_deadline, description = values
            effective_deadline = (
        ...
        if mode == "sony_exposure_sequence":
            seq, prepared_deadline, description, iso = values
            effective_deadline = (
                deadline if deadline is not None
                else prepared_deadline
            )
            self.set_exposure_settings(iso=str(iso))
            return self._execute_sequence(
                seq,
                effective_deadline,
                description,
        ...
        if mode == "sony_exposure_singles":
            plan, prepared_deadline, description = values
            effective_deadline = (
                deadline if deadline is not None
                else prepared_deadline
            )
            return self._execute_exposure_plan_singles(
                plan,
                effective_deadline,
                description,
            )
        ...
        if mode == "sony_exposure_mixed":
            segments, prepared_deadline, description = values
            effective_deadline = (
                deadline if deadline is not None
                else prepared_deadline
            )
            return self._execute_exposure_mixed(
                segments,
                effective_deadline,
                description,
            )
    ...
    def _execute_exposure_mixed(
        self,
        segments,
        deadline,
        description,
    ):
        """Execute contiguous ISO segments using brackets when exact."""

        planned = sum(
            self._planned_count(sequence)
            for sequence, _iso, _description in segments
        )
        total = 0
        current_iso = None

        self.log(
            f"   [sony] plan {description} : "
            f"{len(segments)} segment(s), {planned} vue(s)"
        )

        for sequence, iso, segment_description in segments:
            iso_value = str(iso)
        ...
        return CaptureResult(
            frames=total,
            planned=planned,
            detail="materialized hybrid exposure plan",
        )

    def _execute_exposure_plan_singles(
        self,
        plan,
        deadline,
        description,
        ...
        self.log(
            f"   [sony] plan {description} : "
            f"{planned} vue(s) exacte(s)"
        )

        for speed, iso in plan:
            single = planner.SinglePhoto(speed)

            if deadline is not None:
                remaining = seconds_until_deadline(deadline)
                if (
                    remaining
                    < planner.estimate_duration(single)
            ...
            got = self._fire_single(
                speed,
                deadline=deadline,
            )
            self.log(f"   [sony] PHOTO {speed} : {got}/1")
            total += got

        return CaptureResult(
            frames=total,
            planned=planned,
            detail="materialized exact singles",
        )
