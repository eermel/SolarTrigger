...
def test_load_camera_timing_profile(tmp_path):
    path = _write(
        tmp_path,
        {
            ...
            "timing": {
                ...
                "set_shutter_ms": 153,
                "trigger_single_lead_ms": 254,
                "trigger_single_duration_ms": 333,
                "bracket_press_lead_ms": 285,
    ...
    assert profile.trigger_single_lead_ms == 254
    assert profile.trigger_single_duration_ms == 333
    assert profile.bracket_press_lead_ms == 285
...
def test_missing_values_default_to_zero(tmp_path):
    ...
    assert profile.bracket_press_lead_ms == 0
