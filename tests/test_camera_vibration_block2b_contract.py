...
# ---------------------------------------------------------------------------
# 1/500 HARD SAFETY OUTSIDE TOTALITY
# ---------------------------------------------------------------------------
...
def test_exactly_1_500_is_allowed_outside_totality(phase):
    _validate_non_totality_exposure_plan(
        rig_id=1,
        phase=phase,
        exposure_plan=[
...
def test_slower_than_1_500_is_rejected_outside_totality(
    phase,
    unsafe_shutter,
):
    with pytest.raises(
        ValueError,
        match="slower than 1/500 s outside totality",
    ):
        _validate_non_totality_exposure_plan(
            rig_id=2,
            phase=phase,
...
def test_totality_keeps_long_exposures(shutter):
    _validate_non_totality_exposure_plan(
        rig_id=1,
        phase="totality",
        exposure_plan=[
